module DecisionTree
  ...
  struct Node
    property attribute, threshold, gain

    def initialize(@attribute : String, @threshold : Int32, @gain : Int32 )
    end
  end

  class ID3Tree
    def initialize(attributes : Array(String), data : Array(Array(Float64 | String) | Array(Int32 | String)), default : String, type : Symbol)
    ...
    def train(data = @data, attributes = @attributes, default = @default)
      ...
      data2 = data.reduce({} of Array(Array(Float64 | String) | Array(Int32 | String)) => Hash(String, Float64)) do |hash, d|
        position = d[0..-2]
        hash[position] ||= Hash(String, Float64).new
        hash[position]["#{d.last}"] =+ 1.0
        hash
      end
    ...
    def id3_train(data, attributes, default, _used={} of String => Array(Int32))
...
#    end
    ...
    private def build_tree(tree = @tree)
      return [] of Int32 unless tree.is_a?(Hash)
  ...
  class Rule
    ...
    def initialize(attributes, premises = [] of Int32, conclusion = nil)
      @attributes = attributes
      @premises = premises
